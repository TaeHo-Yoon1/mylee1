...
background_slot = pygame.transform.scale(pygame.image.load(f'background_images/background_1.png'),
                                         (SCREEN_WIDTH, SCREEN_HEIGHT))
background_save = pygame.transform.scale(pygame.image.load(f'background_images/background_2.png'),
                                         (SCREEN_WIDTH, SCREEN_HEIGHT))
...
eyes_images = [pygame.transform.scale(pygame.image.load(f'images/eyes_{i}.png'), (700, 350)) for i in range(1, 6)]
nose_images = [pygame.transform.scale(pygame.image.load(f'images/nose_{i}.png'), (600, 460)) for i in range(1, 6)]
mouth_images = [pygame.transform.scale(pygame.image.load(f'images/mouth_{i}.png'), (800, 360)) for i in range(1, 6)]
background_base = pygame.transform.scale(pygame.image.load(f'background_images/base.png'), (685, 364))
...
show_face_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 100, 200,
                                    50)  # Button to show the completed face
...
def draw_face(hair_pos=(SCREEN_WIDTH // 2, 50), eye_pos=(SCREEN_WIDTH // 2, 175), nose_pos=(SCREEN_WIDTH // 2, 275),
              mouth_pos=(SCREEN_WIDTH // 2, 375)):
    # Draw background
    screen.blit(background_slot, (0, 0))

    # Draw hair
    screen.blit(hair_images[rolling_hair], (hair_pos[0] - hair_images[rolling_hair].get_width() // 2 - 240, hair_pos[1] + 180))
    # Draw eyes
    screen.blit(eyes_images[rolling_eyes], (eye_pos[0] - eyes_images[rolling_eyes].get_width() // 2 - 70, eye_pos[1] + 30))
    # Draw nose
    screen.blit(nose_images[rolling_nose],
                (nose_pos[0] - nose_images[rolling_nose].get_width() // 2 + 80, nose_pos[1] - 120 ))
    # Draw mouth
    screen.blit(mouth_images[rolling_mouth],
                (mouth_pos[0] - mouth_images[rolling_mouth].get_width() // 2 + 235, mouth_pos[1] - 250))
...
def complete_draw_face(hair_pos=(SCREEN_WIDTH // 2, 50), eye_pos=(SCREEN_WIDTH // 2, 175),
    ...
    # Draw face
    background_pos = ((SCREEN_WIDTH - background_base.get_width()) // 2,
                      (SCREEN_HEIGHT - background_base.get_height()) // 2)

    # Draw background
    screen.blit(background_base, background_pos)

    # Draw hair
    screen.blit(hair_images[rolling_hair], (hair_pos[0] - hair_images[rolling_hair].get_width() // 2, hair_pos[1] + 65))
    # Draw eyes
    screen.blit(eyes_images[rolling_eyes], (eye_pos[0] - eyes_images[rolling_eyes].get_width() // 2, eye_pos[1] - 53))
    # Draw nose
    screen.blit(nose_images[rolling_nose], (nose_pos[0] - nose_images[rolling_nose].get_width() // 2, nose_pos[1] - 210))
    # Draw mouth
    screen.blit(mouth_images[rolling_mouth],
                (mouth_pos[0] - mouth_images[rolling_mouth].get_width() // 2, mouth_pos[1] - 255))


def save_face_image(directory="save_images", filename="completed_face.png"):
    """Saves the current screen content as a PNG file in the specified directory."""
    ...
    print(f"Saved face as {file_path}")
...
def draw_notification():
    if notification_message and pygame.time.get_ticks() - notification_time < 2000:  # Show notification for 2 seconds
        ...
        screen.blit(notification_text,
                    (SCREEN_WIDTH // 2 - notification_text.get_width() // 2, SCREEN_HEIGHT // 2 + 100))
...
def show_completed_face():
    ...
    pygame.display.flip()
    save_face_image()  # Save the face as a PNG file in "save_images" directory
...
while running:
    ...
    # Fill the screen with the slot background
    screen.blit(background_slot, (0, 0))
